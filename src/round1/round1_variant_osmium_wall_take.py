...
class Trader:
    ...
    PEPPER_TAKE_THRESHOLD = 4.0
    PEPPER_MAKE_EDGE = 3.0
    ...
    def _trade_osmium(self, depth: OrderDepth, position: int) -> List[Order]:
        ...
        wall_mid = self._osmium_wall_mid(depth)
        ...
        if wall_mid is not None:
            for ask_price, ask_volume in sorted(depth.sell_orders.items()):
                ...
                if ask_qty <= 0:
                    continue
                threshold = 0.0 if position < -30 else 2.0
                if ask_price <= wall_mid + threshold:
            ...
            for bid_price, bid_volume in sorted(depth.buy_orders.items(), reverse=True):
                ...
                if bid_qty <= 0:
                    continue
                threshold = 0.0 if position > 30 else 2.0
                if bid_price >= wall_mid - threshold:
    ...
    def _osmium_wall_mid(self, depth: OrderDepth) -> Optional[float]:
        bid_candidates = [price for price, volume in depth.buy_orders.items() if volume >= 10]
        ask_candidates = [price for price, volume in depth.sell_orders.items() if abs(volume) >= 10]
        if not bid_candidates or not ask_candidates:
            return None
        return (max(bid_candidates) + min(ask_candidates)) / 2.0
